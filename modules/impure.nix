...
let
  ...
  symlinkFiles = pipe cfg.parseAttrs [
    (filter (x: x ? source && x.source != null && cfg.dotsDir != null && hasPrefix (toString cfg.dotsDir) (toString x.source)))
    (filter (x: assertMsg (pathExists x.source) "hjem-impure: the path ${x.source} DOES NOT EXIST"))
    (map (x: "symlink \"${cfg.dotsDirImpure}${removePrefix (toString cfg.dotsDir) (toString x.source)}\" \"${x.target}\""))
    (concatStringsSep "\n")
  ];
...
in
{
  options.modules.impure = {
    ...
    dotsDirImpure = ss.mkOpt types.str config.home.flakeDir;
